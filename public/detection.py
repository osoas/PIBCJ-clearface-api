...
def calculate_iga(detections):
    ...
    iga_score = sum(acne_weights[cls] * acne_count.get(cls, 0) for cls in acne_weights)
    return iga_score

def get_next_folder(base="detection"):
    """Finds the next available folder name inside 'detections/'."""
    os.makedirs("detections", exist_ok=True)  
    existing_folders = sorted(glob.glob(f"detections/{base}*"))
    
    if not existing_folders:
        return f"detections/{base}01"
    
    last_number = max(
        [int(folder.replace(f"detections/{base}", "")) for folder in existing_folders if folder.replace(f"detections/{base}", "").isdigit()],
        default=0
    )
...
def draw_crosshairs(image_path, boxes):
    ...
    colors = [
        "#ADD8E6", "#87CEEB", "#4682B4", "#4169E1", "#0000FF", "#8A2BE2", "#9400D3", "#A52A2A", "#B22222", "#FF0000"
    ]
...
def process_image(image_path, model, conf=.25):
